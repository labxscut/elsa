...
def main():
  ...
  for j in range(0, simTimes):
    ...
    if not trendSeries:
      ...
      xSeries = lsalib.ji_calc_trend_rep(OxSeries, trend_threshold)
      ySeries = lsalib.ji_calc_trend_rep(OySeries, trend_threshold)
    ...
    if not trendSeries:
      lsa_result = lsalib.singleLSA(xSeries, ySeries, delayLimit, lsalib.simpleAverage, \
          lsalib.percentileZNormalize, True)
      LS_values[j] = np.abs( lengthSeries * lsa_result.score )
      P_perm[j] = lsalib.permuPvalue(xSeries, ySeries, delayLimit, int(1/perm_precision), \
          LS_values[j]/lengthSeries, lsalib.simpleAverage, lsalib.percentileZNormalize)
    else:
      lsa_result = lsalib.singleLSA(xSeries, ySeries, delayLimit, lsalib.simpleAverage, \
          lsalib.noneNormalize, True)
      LS_values[j] = np.abs( lengthSeries * lsa_result.score )
      P_perm[j] = lsalib.permuPvalue(xSeries, ySeries, delayLimit, int(1/perm_precision), \
          LS_values[j]/lengthSeries, lsalib.simpleAverage, lsalib.noneNormalize)
  ...
  print >>resultFile, '\n'.join(['\t'.join( \
    ...
    str(u1[i]),str(u2[i]),str(v1[i]),str(v2[i]),str(Xs[i]),str(Ys[i]),str(D[i]),str(Al[i])]
